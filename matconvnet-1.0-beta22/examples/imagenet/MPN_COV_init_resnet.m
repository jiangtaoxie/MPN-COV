...
for s = 2 : 5
  ...
  for l = 1:sectionLen(s)
    ...
    Conv([name 'a'], 1, 2^(s+4), 'downsample', (s >= 3 & s < numel(sectionLen)) & l == 1) ; % no downsampling for the last section
    Conv([name 'b'], 3, 2^(s+4)) ;
...
conv_for_cov_dim = 256;
Conv([name 'a'],  1,  conv_for_cov_dim,  'downsample',  false,  'relu',  true,  'bias',  true) ;

name = 'mpn_cov_pool';
net.addLayer(name , ...
                     dagnn.MPN_COV_Pool_C('method',  [],   'regu_method', 'power',   'alpha', 0.5), ...
                     lastAdded.var, ...
...
lr = [0.1 * ones(1,30), 0.01*ones(1,30), 0.001*ones(1,30)] * 10^-0.2;
